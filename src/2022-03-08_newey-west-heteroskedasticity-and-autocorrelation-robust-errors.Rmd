...
```{r}
data_with_white_noise_error <- function(iterations = 2, 
                                        n_values = c(50, 100)){
  # Create a dataframe of simulated X and Y data, iterating over several
  # sample sizes. Each iteration is a single sample for the given parameters.  
  ...
  index <- 0
  for (n in n_values){
    white_noise_residuals <- rnorm(n * i)
    x <- rep(seq(n), i)
    y <- slope * x + white_noise_residuals
    
    x_collected <- append(x_collected, x)
    y_collected <- append(y_collected, y)
    sample_id <- append(sample_id, rep(seq(1 + index:i + index), each = n))
...
# df_white_noise <- data_with_white_noise_error(iterations = i, n_values = c(50, 100), ar_coeffecients = seq(0, .9, .2))

# df_arima_error <- data_with_arima_error(iterations = i, n_values = c(50, 100), ar_coeffecients = seq(0, .9, .2))

# df_arima <- data_with_arima_response(iterations = i, n_values = c(50, 100), ar_coeffecients = seq(0, .9, .2))
